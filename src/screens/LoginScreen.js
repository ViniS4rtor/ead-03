...
export default function LoginScreen({ navigation }) {
  ...
  const listarUsuarios = async () => {
    try {
      ...
      const listaUsuarios = usuarios
        .map((usuario) => `• ${usuario.nome} (${usuario.email})`)
        .join("\n");

      Alert.alert(`Usuários Registrados (${usuarios.length})`, listaUsuarios, [
        { text: "OK" },
      ]);
    } catch (error) {
  ...
  const camposPreenchidos = useMemo(() => {
    return (
      email.trim().length > 0 && senha.trim().length > 0 && validarEmail(email)
    );
  }, [email, senha]);
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
        style={[
          styles.input,
          email.length > 0 && !validarEmail(email) && styles.inputError,
        ]}
      ...
      {email.length > 0 && !validarEmail(email) && (
        <Text style={styles.errorText}>
          Email deve conter @ e ter formato válido
        </Text>
      )}
      ...
      <View style={styles.buttonArea}>
        <TouchableOpacity
          style={[
            styles.button,
            camposPreenchidos ? styles.buttonEnabled : styles.buttonDisabled,
          ]}
      ...
      <View style={styles.linksContainer}>
        ...
        <Pressable onPress={listarUsuarios}>
          <Text style={[styles.link, styles.listUsersLink]}>
